	...
	public void start_MainGame() {
		...
		for (Player player : joined_players) {
			player.sendMessage(Component.text(
					Feedback.get_msg("event.start_final", " place a block in the dimension '" + TextEditing.get_clean_dimensionName(location.getWorld().getEnvironment()) + "' at the coordinates " + TextEditing.get_clean_coordinates(location)))
